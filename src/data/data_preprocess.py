...
class AudioPreprocessor(PreporcessorStrategy):
    def process(
        ...
        **kwargs,
        ...
        try:
            audio = dataset["audio"]
            audio_array, sampling_rate = self._load_audio_array(
                audio, target_sampling_rate=args.sampling_rate
            )
    ...
    @staticmethod
    def _load_audio_array(audio: Dict[str, Any], target_sampling_rate: int):
        if not isinstance(audio, dict):
            raise ValueError(f"Unexpected audio type: {type(audio)}")

        # When datasets Audio feature is decoded we receive 'array'/'sampling_rate'.
        ...
        if audio_bytes is not None:
            with BytesIO(audio_bytes) as buffer:
                audio_array, sampling_rate = librosa.load(
                    buffer, sr=target_sampling_rate
                )
            return audio_array, sampling_rate

        raise ValueError("Audio sample must contain either 'array', 'path', or 'bytes'.")
...
class TextPreprocessor(PreporcessorStrategy):
    ...
    def _process_timestamp(self, target_text, dataset, args):
        if isinstance(target_text, list):
            ...
            audio = dataset["audio"]
            audio_array, _ = AudioPreprocessor._load_audio_array(
                audio, target_sampling_rate=args.sampling_rate
            )
