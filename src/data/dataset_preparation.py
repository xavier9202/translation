...
class AudioDatasetPreparationStrategy(DatasetPreparationStrategy):
    ...
    def _resample_audio(self):
        ...
        audio_feature = self.dataset.features.get("audio")
        ...
        if current_sampling_rate != target_sampling_rate:
            ...
            logger.info(
                f"Ensuring audio column uses sampling rate {target_sampling_rate} Hz"
            )
        # Use decode=False so we can load audio with librosa/soundfile and avoid torchcodec dependency.
    ...
    def _standardize_features(self):
        target_features = Features(
            {
                "audio": Audio(sampling_rate=self.args.sampling_rate, decode=False),
                "target": [
                    {
